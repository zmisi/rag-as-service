...
- 文件：`.txt` / `.pdf` / `.docx` / `.pptx`；单文件 ≤ 20MB（不支持旧版 `.doc` / `.ppt`）
...
| `Q3_update.pptx` | 2026 Q3 产品更新说明 |
| `refund_policy.docx` | 退款政策与操作流程 |
...
| 规则 | UI 行为 |
|------|---------|
| 允许扩展名 | `.txt` `.md` `.pdf` `.docx` `.pptx` |
| 单文件 ≤ 20MB | 超限拒绝并提示（F03-T08） |
| 不支持类型（如 `.exe`） | 拒绝并提示（F03-T07） |
| 旧版 `.doc` / `.ppt` | 拒绝并提示另存为 `.docx` / `.pptx`（F03-T07b） |
