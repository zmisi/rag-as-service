...
flowchart TD
  ...
  C --> D[拉取源文件]
  D --> E{解析成功?}
  E -->|否| E1[job=failed / ingest_status=failed 可重试]
  E -->|是| S[构建 H1-H6 Section 树]
  S --> F{有可索引文本?}
  F -->|否 空文档/无字 PDF| I0[job=succeeded / ingest_status=ready 0 section/chunk]
  F -->|是| G[节内切 leaf ChunkDraft]
  G --> H[仅 leaf Embedding]
  H --> W[写入 sections + chunks 绑定 tenant_id is_latest=true]
  W --> I[job=succeeded / ingest_status=ready]
  I --> J[该版本行可被 search]
...
1. **门禁（写）**：`publish_status != published` 的文档不得产生可检索态（`is_latest=true`）的 section / chunk。Publish 入队时版本行 `ingest_status=pending`。
2. **门禁（读 / search）**：仅当文档同时满足 `publish_status=published` AND `ingest_status=ready` AND `deleted_at IS NULL` AND leaf/section **`is_latest=true`**，且强制 `tenant_id` 过滤，方可命中。
...
5. Worker 状态：抢到 job → `ingest_status=processing`；成功 → `ready`（并写 documents 上 embedding 审计字段）；失败 → `failed` + `error_message`；job 表 `status` 仍维护（队列视角）。
6. **解析失败**（文件损坏、PyMuPDF 与 Docling 均无法打开/转换等）：job `failed`，文档仍 `published`，`ingest_status=failed`，无 `is_latest` section/chunk；可重试（Phase 1：至少一条可测重试路径）。
7. **无字 / 空文档**：空 txt、或无文字层且未做 OCR 的 PDF → 解析结果为空 → job **`succeeded`**，`ingest_status=ready`，**0** section/chunk；search 无命中。（与「损坏失败」区分。）
8. **解析与表结构**：
...
12. **检索契约**（本 Feature 实现）：`search(tenant_id, query, top_k) → Hit[]`
    - 用 query embedding 在 **`is_latest` leaf** 上 top-k（且对应文档 `publish_status=published`、`ingest_status=ready`、未软删）；
...
| 实体 | 关键字段 / 约束 |
|------|----------------|
| documents（版本行，索引相关） | `doc_id`, `tenant_id`, `doc_group_id`, `version_number`（int）, `is_latest`, `publish_status`, `ingest_status`(`pending`\|`processing`\|`ready`\|`failed`), `error_message`, `embedding_*`（审计） |
| ingest_job | `id`, `tenant_id`, `doc_id`（→ 版本行）, `version`（int）, `status`(`pending`\|`running`\|`succeeded`\|`failed`), `error` |
...
**检索门禁**：`publish_status=published` AND `ingest_status=ready` AND `deleted_at IS NULL` AND section/chunk `is_latest=true` AND `tenant_id` 匹配。
