...
> 将 F03/F04 持久化对齐为多租户**文档版本行** + 双状态（`publish_status` / `ingest_status`）+ 统一 `is_latest` + 富 `document_chunks`；字符串列一律 `text`；schema 仍为 `rag_service`。
...
| ID | 步骤 | 期望 | 类型 |
|----|------|------|------|
| F07-T01 | Given 迁移已应用 When 检查 `documents`/`document_sections`/`document_chunks`/`ingest_jobs` 列类型 | Then 字符串列为 `text`（无 `varchar`）；存在 `create_at`/`update_at` 与 `tr_*_lmt`；`documents.version_number` 为 int；存在 `file_storage_path`/`file_name`/`file_content_type`；**无** `document_files` 表 | unit |
...
| F07-T04 | Given 版本行 publish 且 index job 入队 When 读库 | Then `publish_status=published` 且 `ingest_status` 为 `pending` 或 `processing` | api |
| F07-T05 | Given 已 published 版本 When 索引失败 | Then `publish_status` 仍为 `published`；`ingest_status=failed`；`error_message` 非空 | api |
| F07-T06 | Given 索引成功 When 读 documents 与 chunks | Then `ingest_status=ready`；documents 上 `embedding_model`/`embedding_dimension`（及 provider 若有）已填；chunks **无** `embedding_model` 列 | api |
...
| F07-T08 | Given 同租户另一 `doc_group` 已有 `file_content_sha256` 且 `published`+`ready`+`is_latest` When 再 publish 相同内容 | Then HTTP **409**；`detail.code=duplicate_content_sha256`；`detail.existing_document_id`/`existing_title` 指向已有文档；本版回退为 `draft`；无本版 ingest_job / 无可检索 chunk | api |
| F07-T09 | Given tenant-A 与 tenant-B 发布相同 `content_sha256` When 双方索引 | Then 各自拥有独立版本行与可检索 chunk；互不 search 命中对方内容 | api |
| F07-T10 | Given 摄入 worker 写入 H1/H2 节 When 读 sections | Then 每节 `section_index` 在 `document_id` 内唯一；`level` 为 text（如 `'1'`/`'2'`） | api |
| F07-T11 | Given 摄入 worker 写入 leaf When 读 chunks | Then 含 `tenant_id`、单调 `chunk_index`、非空 `embedding_text`、`heading_path` 与父节对齐、`is_latest=true`；无 `embedding_model` | api |
...
| F07-T14 | Given F07 迁移与适配完成 When 跑 F04-T01–T15（及 F03 publish 关键路径） | Then 在新列名（`is_latest`/`ingest_status`/`section_index`/`chunk_index`/`version` int）下全部通过 | api |
