...
- Widget 内发问走后端（经 site key + Origin 校验），**复用 F06 Agent**（`run_user_turn`）；语义对齐 F06 chat（**不**要求 F11 `rk_live_`；**不**把服务端 Key 下发给浏览器）
...
- F11 `{subdomain}.lxzxai.com/api` 与 `rk_live_`（本 Feature 不实现）
...
flowchart TD
  ...
  C -->|是| D[渲染浮层聊天 UI]
  D --> E[用户发问]
  E --> F[F06 run_user_turn 本租户]
  F --> G[JSON 或 SSE 回复]
```

## 行为规则

1. Site key 绑定 `tenant_id`；与 F11 `api_key` 分表（F11 未实现前仅有本表）；可独立吊销。
...
| F12-T08 | Given 租户 Host When GET `/widget` | Then 页面含 widget snippet（`widget.js` + `data-site-key` 占位或真 key） | e2e |
