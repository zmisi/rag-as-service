...
2. Office HTML 预览：**只读**；生成失败 → 4xx/502 + 可重试；成功后按 `file_content_sha256`（或 file id + size + mtime）缓存，文件替换后失效。
3. PDF 预览 URL 仅当前会话可访问（或短 TTL 签名，≤15min）；禁止无鉴权永久外链。
4. 单次预览响应体上限与文件 50MB 上传上限一致；超大转换超时（固定 **60s**）→ 失败可测。
5. 未上传文件 / 已软删 → 404。
...
| 实体 | 关键字段 / 约束 |
|------|----------------|
| preview_cache（可选表或对象存储元数据） | `doc_id`, `tenant_id`, `format=html`, `file_storage_path`, `source_fingerprint` |
