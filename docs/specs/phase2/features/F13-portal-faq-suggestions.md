...
7. 租户隔离；Portal 需按现有 F05/F06 登录策略（与 Phase 1 一致：成员登录后使用）。
...
| ID | 步骤 | 期望 | 类型 |
|----|------|------|------|
| F13-T01 | Given 租户有 ≥5 条 published FAQ 且 2 条 `is_hot` When GET suggestions | Then 返回 5 条；前 2 条 `hot=true`；后 3 条 `hot=false` | api |
| F13-T02 | Given 有 `is_hot` 条且非 Hot 的 click 更高 When 展示 | Then `is_hot` 条仍排在非 Hot 之前且带 hot | api |
| F13-T03 | Given 点击某条 FAQ When 计数 | Then 该条 `click_count` +1；`is_hot` 不变；可触发聊天内容=题面 | api |
| F13-T04 | Given 有 Hot + 多条非 Hot When 换一批 | Then Hot 仍在前；下方非 Hot 变为下一段（或循环） | api |
